...
Portworx provides volume\(s\) to the elastic search data nodes. Create `portworx-sc.yaml` with Portworx as the provisioner and apply the configuration. This storage class creates a replica 2 Portworx volume when reference via a PersistentVolumeClaim.
...
#### Verify Elastic Search installation. {#verify-elastic-search-installation}
...
#### Verify Elastic search cluster state. {#verify-elastic-search-cluster-state}
...
#### Verify Kibana Installation {#verify-kibana-installation}
...
Once you have run the above command you should see `bank` and `customer` indices in your elasticsearch cluster. Search for them through your Kibana dashboard.
...
#### Pod Failover for Elastic search {#pod-failover-for-elastic-search}

Portworx provides durable storage for the Elastic search pods. Cordon a node so that pods do not get scheduled on it, delete a pod manually to simulate a failure scenario and watch the pod get scheduled on another node. However the Statefulset with PX as the volume would reattach the
